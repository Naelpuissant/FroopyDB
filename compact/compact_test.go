...
func TestMaybeCompactToUpperLevel(t *testing.T) {
	...
	t1.Open()
	t1.InitWriter()
	t1.WriteDataBlock([]byte{1}, []byte("a"))
	...
	t2.Open()
	t2.InitWriter()
	t2.WriteDataBlock([]byte{4}, []byte("D"))
	idxOffset, _ = t2.WriteIndex()
	t2.WriteMetadata(idxOffset)
	t2.FlushWriter()
	defer t2.Close()
	...
	t3.Open()
	t3.InitWriter()
	t3.WriteDataBlock([]byte{1}, []byte("a"))
	...
	t4.Open()
	t4.InitWriter()
	t4.WriteDataBlock([]byte{2}, []byte("B"))
	...
	t5.Open()
	t5.InitWriter()
	t5.WriteDataBlock([]byte{1}, []byte("A"))
	t5.WriteDataBlock([]byte{3}, []byte("C"))
	t5.WriteDataBlock([]byte{5}, []byte("E"))
	idxOffset, _ = t5.WriteIndex()
